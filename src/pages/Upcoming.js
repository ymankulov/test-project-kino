...
const Upcoming = () => {
    ...
    useEffect(() => {
        axios(`https://api.themoviedb.org/3/movie/upcoming?api_key=${apikey}&language=en-US&page=${currentPage}`)
            .then(({data}) => setUpcoming(data.results))
    }, [currentPage])
    return (
        <div className="container p-5 bg-blue-300">
            <h1 className='text-5xl text-center text-blue-900 mb-5  hover:scale-110 hover:transition hover:text-gray-50'>Upcoming Movies:</h1>
            {
                upcoming.length === 0 ? <SecondLoader/> : <div className=" flex flex-row flex-wrap">
                    {
                        upcoming.map(el => (
